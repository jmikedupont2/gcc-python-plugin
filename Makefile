...
PYTHON=/usr/local/bin/python3.11d
...
PYTHON_LIBS=-lpython3.11d  -ldl  -lm  \
...
#
INVOCATION_ENV_VARS := PYTHONPATH=$(srcdir)./:/testing/gccpy CC_FOR_CPYCHECKER=$(CC) LD_LIBRARY_PATH=gcc-c-api:$(LD_LIBRARY_PATH):/usr/local/lib/ CC=$(CC)
...
# A simple demop, to make it easy to demonstrate the cpychecker:
demo: demo.c plugin $(pwd)/gcc-with-cpychecker
	echo $(INVOCATION_ENV_VARS) ./gcc-with-cpychecker -c $(PYTHON_INCLUDES) $<
